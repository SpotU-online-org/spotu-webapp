...
export default async function ListingPage({ params }: PageProps) {
  ...
  const { data: listing } = await supabase
    ...
    .select(`
      *,
      profile:profiles(id, display_name, company_name, avatar_url, bio, is_verified, confirmed_interactions_count)
    `)
    .eq("id", id)
    .eq("status", "active")
    .single();
  ...
  const config = TYPE_CONFIG[listing.type as keyof typeof TYPE_CONFIG];
                    ...
                    <div className="sm:col-span-2">
                      ...
                      <dd className="flex flex-wrap gap-2">
                        {listing.coverage_areas.map((a: string) => (
                          <span key={a} className="rounded-full bg-muted px-2.5 py-0.5 text-xs font-medium text-foreground">
                            {a}
                          </span>
                        ))}
                      </dd>
            ...
            <aside className="space-y-5">
              ...
              {profile && (
                <div className="rounded-2xl border bg-card p-5">
                  ...
                  {typeof profile.confirmed_interactions_count === "number" &&
                    profile.confirmed_interactions_count > 0 && (
                      <p className="mt-3 text-xs text-muted-foreground">
                        <span className="font-semibold text-foreground">
                          {profile.confirmed_interactions_count}
                        </span>{" "}
